...
def train_NOmeta(args):
    ...
    corpus_en_train = Corpus('bert-base-multilingual-cased', args.max_seq_len, logger, language='en', mode='46',
    ...
    for step in range(args.max_meta_steps):
        ...
        if  step ==14:   #step % args.eval_every_meta_steps == 0: ste
...
def train_meta(args):
    ...
    corpus_en_train = Corpus('bert-base-multilingual-cased', args.max_seq_len, logger, language='en', mode='train',
                             load_data=False, support_size=args.support_size, base_features=None, mask_rate=args.mask_rate,
...
def k_shot(args):
    ...
    for lang in languages:
        corpus_train = Corpus('bert-base-multilingual-cased', args.max_seq_len, logger, language=lang, mode='thesis_train_i_1',
                              load_data=False, support_size=-1, base_features=None, mask_rate=-1.0,
        ...
        for ft_step in range(args.max_ft_steps):
            ...
            if ft_step in [0,4,9,15,20,25]:
                ...
                #print((corpus_test.size))
                #for i,x in enumerate(corpus_test):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--eval_every_meta_steps', type=int, default=100)
