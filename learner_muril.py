...
class Learner(nn.Module):
    def __init__(self, bert_model, label_list, freeze_layer, logger, lr_meta, lr_inner,
        ...
        if model_dir != '':
            ...
            self.model = AutoModelForTokenClassification.from_pretrained(self.bert_model, from_tf = True,cache_dir=cache_dir, num_labels=num_labels)
...
#model_recover = AutoModel.from_pretrained(self.bert_model, from_tf = True)
    ...
    def inner_update(self, data_support, lr_curr, inner_steps, lambda_max_loss, lambda_mask_loss):
        ...
        for i in range(inner_steps):
            ...
            inner_opt.zero_grad()
            loss = self.model(data_support['input_ids'], data_support['segment_ids'],
    ...
    def forward_meta(self, batch_query, batch_support, progress, inner_steps, lambda_max_loss, lambda_mask_loss): # for one task
        ...
        for task_id in range(task_num):
            ...
            loss = self.model(batch_query[task_id]['input_ids'], batch_query[task_id]['segment_ids'],
    ...
    def forward_NOmeta(self, batch_data, lambda_max_loss, lambda_mask_loss): #, lambda_flag=-1.0):
        ...
        loss = self.model.forward_wuqh(batch_data['input_ids'], batch_data['segment_ids'],
                                       batch_data['input_mask'], batch_data['label_ids']) #, lambda_flag=lambda_flag)
        loss.backward()
        self.opt.step()
    ...
    def write_result(self,  y_true, y_pred, tmp_fn):
    ...
    def evaluate_meta(self, corpus, result_dir, logger, lr, steps, lambda_max_loss, lambda_mask_loss, lang='en', mode='valid'):
        ...
        self.write_result( y_true, y_pred, tmp_fn)
    ...
    def evaluate_NOmeta(self, corpus, result_dir, logger, lang='en', mode='valid'):
        ...
        for batch_id, data_batch in enumerate(data_batches):
            ...
            with torch.no_grad():
                logits_0 = self.model(data_batch['input_ids'], data_batch['segment_ids'],
                               data_batch['input_mask'])  # batch_size x seq_len x target_size
            logits_0 = logits_0[0]
            
            logits = torch.argmax(torch.nn.functional.log_softmax(logits_0, dim=2), dim=2)
            logits = logits.detach().cpu().numpy()
            #print(logits)
                ...
                for j, m in enumerate(mask):
                    ...
                    if m:
                        if label_map[label_ids[i][j]] != "X":
                            temp_1.append(label_map[label_ids[i][j]])
                            temp_2.append(label_map[max(logits[i][j], 1)])
                            #print((label_map[max(logits[i][j], 1)]))
                            #print("------------------------------------------------------------------")
                          
                            #print(label_map[label_ids[i][j]])

                            #temp_word.append(tokenizer.convert_ids_to_tokens(input_ids[i][j]))
                        else:
                            print("ffff")
                            #tmp = tokenizer.convert_ids_to_tokens[input_ids[i][j]]
                            #if len(tmp) > 2 and len(temp_word) > 0:
                                #temp_word[-1] = temp_word[-1] + tmp[2:]
                    else:
                        temp_1.pop() # pop [SEP]
        ...
        self.write_result( y_true, y_pred, tmp_fn)
        os.system('%s %s < %s > %s' % (self.py_alias, 'conlleval.py', tmp_fn, score_fn))
        f1_results_event= f1_score(y_true,y_pred)
        #print("-------------------------------------------------")
        print(f1_results_event)
        F1 = -1
        with open(score_fn, 'r', encoding='utf-8') as fr:
            for id, line in enumerate(fr):
                if id == 1:
